...
# three ways for learning classification for Iris Setosa
...
X = iris["data"][:, (1, 2)] #Sepal width, petal length
y = (iris["target"] == 0).astype(np.float64).reshape(-1, 1) # Iris-Setosa
...
plot_svc_decision_boundary(svm_clf, 1, 9)
...
plot_svc_decision_boundary(svm_clf2, 1, 10)
plt.xlabel("Sepal Width", fontsize=14)
...
plt.plot(X[:, 0][yr==1], X[:, 1][yr==1], "g^")
plt.plot(X[:, 0][yr==0], X[:, 1][yr==0], "bs")
plot_svc_decision_boundary(sgd_clf, 1, 9)
plt.xlabel("Sepal Width", fontsize=14)
plt.ylabel("Petal Length", fontsize=14)
